"""RLAP SQLite persistence for sessions and actions."""
...
CREATE INDEX IF NOT EXISTS idx_app_sessions_contact
    ON app_sessions(contact_hash, identity_id);
CREATE INDEX IF NOT EXISTS idx_app_sessions_status
    ON app_sessions(status);
...
class RlapStore:
    """SQLite-backed storage for RLAP sessions and actions."""
    ...
    def get_session(self, session_id, identity_id=""):
        ...
        conn = self._get_conn()
        row = conn.execute(
            "SELECT * FROM app_sessions WHERE session_id=? AND identity_id=?",
    ...
    def update_session(self, session_id, identity_id="", **kwargs):
        ...
        conn = self._get_conn()
        conn.execute(
            "UPDATE app_sessions SET {} WHERE session_id=? AND identity_id=?".format(sets),
            vals,
        )
        conn.commit()
    ...
    def list_sessions(self, identity_id="", app_id=None, status=None,
        ...
        conn = self._get_conn()
        rows = conn.execute(
            "SELECT * FROM app_sessions WHERE {} ORDER BY last_action_at DESC".format(
                " AND ".join(clauses)),
            params,
        ).fetchall()
    ...
    def delete_session(self, session_id, identity_id=""):
        ...
        conn = self._get_conn()
        conn.execute(
            "DELETE FROM app_sessions WHERE session_id=? AND identity_id=?",
            (session_id, identity_id),
        )
        conn.execute(
            "DELETE FROM app_actions WHERE session_id=? AND identity_id=?",
            (session_id, identity_id),
        )
        conn.commit()
    ...
    def save_action(self, session_id, identity_id, action_num, command,
        ...
        conn = self._get_conn()
        conn.execute(
            """INSERT OR REPLACE INTO app_actions
    ...
    def get_action_count(self, session_id, identity_id=""):
        ...
        conn = self._get_conn()
        row = conn.execute(
            "SELECT COUNT(*) FROM app_actions WHERE session_id=? AND identity_id=?",
            (session_id, identity_id),
        ).fetchone()
        return row[0]
