"""RLAP envelope packing, unpacking, and validation."""
...
def pack_lxmf_fields(envelope):
    ...
    Returns:
        dict: {0xFB: "rlap.v1", 0xFD: envelope}
    """
    return {
...
def unpack_envelope(fields):
    """Extract and validate an RLAP envelope from LXMF fields.

    Args:
        fields: dict of LXMF fields (keyed by field ID).

    Returns:
        dict: the envelope, or None if not an RLAP message.

    Raises:
        InvalidEnvelope: if fields indicate RLAP but envelope is malformed.
    """
