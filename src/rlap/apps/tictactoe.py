"""RLAP Tic-Tac-Toe app — full implementation with both-side validation."""
...
class TicTacToeApp(AppBase):
    ...
    def __init__(self):
        ...
        # Can be replaced with RlapStore for persistence.
        self._sessions = {}
    ...
    def handle_outgoing(self, session_id, command, payload, identity_id):
        ...
        if command == CMD_DRAW_OFFER:
            return {}, "[RLAP TTT] Offered a draw"
        if command == CMD_DRAW_ACCEPT:
            return self._handle_draw_accept_out(session_id, identity_id)
        if command == CMD_DRAW_DECLINE:
            return {}, "[RLAP TTT] Declined draw offer"
        return payload, "[RLAP TTT] {}".format(command)
    ...
    def render_fallback(self, command, payload):
        if command == CMD_CHALLENGE:
            return "[RLAP TTT] Sent a challenge!"
        if command == CMD_ACCEPT:
            return "[RLAP TTT] Challenge accepted"
        if command == CMD_DECLINE:
            return "[RLAP TTT] Challenge declined"
        if command == CMD_MOVE:
            terminal = payload.get("x", "")
            if terminal == "win":
                return "[RLAP TTT] X wins!" if _marker_for_move(payload.get("n", 0)) == "X" else "[RLAP TTT] O wins!"
            if terminal == "draw":
                return "[RLAP TTT] Game drawn!"
            return "[RLAP TTT] Move {}".format(payload.get("n", "?"))
        if command == CMD_RESIGN:
            return "[RLAP TTT] Resigned."
        if command == CMD_DRAW_OFFER:
            return "[RLAP TTT] Offered a draw"
        if command == CMD_DRAW_ACCEPT:
            return "[RLAP TTT] Draw accepted"
        if command == CMD_DRAW_DECLINE:
            return "[RLAP TTT] Draw declined"
        if command == CMD_ERROR:
            return "[RLAP TTT] Error: {}".format(payload.get("msg", "Unknown"))
        return "[RLAP TTT] {}".format(command)
    ...
    def _handle_resign_in(self, session_id, sender_hash, identity_id):
        ...
        meta = session.metadata
        # The one who didn't resign wins
        meta["terminal"] = "resign"
        # Determine winner: the other player
        if sender_hash == meta.get("first_turn", ""):
            # Challenger (X) resigned, so O wins — that's us
