"""RLAP app registry, discovery, and dispatch."""
...
def get_app(app_id):
    """Get a registered app instance, or None."""
    with _registry_lock:
        return _registry.get(app_id)


def list_apps():
    """Return manifests for all registered apps."""
    with _registry_lock:
        return [app.get_manifest() for app in _registry.values()]


def discover(package):
    """Scan a package for AppBase subclasses and register them.
            ...
            try:
                ...
                for attr_name in dir(mod):
                    ...
                    if (isinstance(cls, type)
                            and issubclass(cls, AppBase)
                            and cls is not AppBase
                            and cls.app_id):
                        instance = cls()
...
def dispatch_incoming(envelope, sender_hash, identity_id=""):
    """Route an incoming RLAP envelope to the correct app handler.

    Returns:
        dict: result from the app's handle_incoming, or None.

    Raises:
        UnknownApp: if no handler is registered for the app.
    """
    ...
    if not handler:
        raise UnknownApp("No handler for app '{}'".format(app_id))
...
def dispatch_outgoing(app_id, command, payload, session_id,
    ...
    if not handler:
        raise UnknownApp("Unknown app: {}".format(app_id))
